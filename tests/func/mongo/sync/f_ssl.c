...
{
  gchar *trusted_fp = g_alloca (128);
  gchar *trusted_dn = g_alloca (128);
  int fd;
  GList *trusted_DNs = NULL;
  GList *trusted_fps = NULL;

  memset (trusted_fp, 0, 128);
  memset (trusted_dn, 0, 128);

  fd = open ("./ssl/3party/server.sign", O_RDONLY);
  read (fd, trusted_fp, 64);
  close (fd);

  fd = open ("./ssl/3party/server.dn", O_RDONLY);
  read (fd, trusted_dn, 128);
  close (fd);
  ...
  ok (conn == NULL &&
    (mongo_ssl_get_last_verify_result (config.ssl_settings) == MONGO_SSL_V_ERR_UNTRUSTED_FP),
  ...
  ok (((conn != NULL) &&
     (mongo_ssl_get_last_verify_result (config.ssl_settings) == MONGO_SSL_V_OK_TRUSTED_FP)),
  ...
  trusted_DNs = g_list_append (trusted_DNs, "Some untrusted DN");
  ...
  ok ((conn == NULL) &&
     (mongo_ssl_get_last_verify_result (config.ssl_settings) == MONGO_SSL_V_ERR_UNTRUSTED_DN),
      "SSL connection fails with untrusted DN");

  trusted_DNs = g_list_append (trusted_DNs, trusted_dn);
  ...
  ok ((conn != NULL) &&
     (mongo_ssl_get_last_verify_result (config.ssl_settings) == MONGO_SSL_V_OK_ALL),
...
test_func_mongo_sync_ssl_insert_query (void)
{
  ...
  ok (mongo_sync_cmd_insert (conn, config.ns, test_doc, NULL) == TRUE,
      "inserting a document works via SSL");
...
ssl_insert_thread (gpointer _c)
{
  ...
  bson_append_string (test_doc, test_string, "ok", -1);
  bson_finish (test_doc);
  ok (mongo_sync_cmd_insert (conn, config.ns, test_doc, NULL),
      "mongo_sync_cmd_insert () works in a multi-threaded scenario over SSL");
  mongo_sync_disconnect (conn);
  bson_free (test_doc);
  g_free (test_string);
}
...
ssl_delete_thread (gpointer _conn) // here, we pass the connection object
{
  ...
  bson_append_string (test_doc, test_string, "ok", -1);
  bson_finish (test_doc);
  ok (mongo_sync_cmd_delete (conn, config.ns, 0, test_doc),
      "mongo_sync_cmd_delete works in a multithreaded scenario");
  bson_free (test_doc);
  g_free (test_string);
  return NULL;
}
...
test_func_mongo_ssl_multithread (void)
{
  ...
  GThreadPool *thread_pool = g_thread_pool_new (ssl_query_thread,
                                                config.ssl_settings,
                                                THREAD_POOL_SIZE,
                                                TRUE,
                                                NULL);
  ...
  srand (time (NULL));
  thread_pool = g_thread_pool_new (ssl_ping_thread,
                                   config.ssl_settings,
                                   THREAD_POOL_SIZE,
                                   TRUE,
                                   NULL);
...
test_func_mongo_ssl_untrusted (void)
{
  ...
  ok (!conn, "By default, invalid certificates are not accepted");
  mongo_ssl_set_security (alternative, FALSE, TRUE);
  conn = mongo_sync_ssl_connect (config.primary_host, config.primary_port, TRUE, alternative);
  ok (!conn, "When certificate is not required, still gets validated if provided");
  ...
  ok (conn, "Connection works in untrusted mode");
  mongo_sync_disconnect (conn);
